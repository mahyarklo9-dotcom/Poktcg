...
async def get_inventory(user_id):
    """برای /inv — برمی‌گردونه (card_name, rarity, card_id, count)"""
    async with aiosqlite.connect(DB_NAME) as db:
        cur = await db.execute(
            """
            SELECT card_name, rarity, card_id, COUNT(*) as cnt
            FROM inventory
            WHERE user_id=?
            GROUP BY card_name
            ORDER BY card_name
...
async def transfer_card(from_user, to_user, card_name):
    async with aiosqlite.connect(DB_NAME) as db:
        cur = await db.execute(
            "SELECT card_id, card_name, rarity, id FROM inventory WHERE user_id=? AND card_name=? LIMIT 1",
            (from_user, card_name)
        )
