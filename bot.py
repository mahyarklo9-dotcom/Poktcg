...
async def _send_inv_card(target, user_id: int, items: list, page: int):
    ...
    total = len(items)
    card_name, rarity, card_id, count = items[page]
    img_url = cards_api.build_image_url(
        f"https://assets.tcgdex.net/en/{'/'.join(card_id.split('-'))}" if card_id else None
    )
    # URL واقعی رو از card_id بسازیم
    # فرمت card_id در TCGdex مثل: swsh3-136
    if card_id and "-" in card_id:
        parts = card_id.split("-", 1)
        img_base = f"https://assets.tcgdex.net/en/{parts[0]}/{card_id}"
